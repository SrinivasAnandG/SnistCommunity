...
<body>
	<?php
		if(isset($_SESSION['user_id'])&&isset($_SESSION['username']))
		{
			...
			$search_value_replace=str_replace(' ','',$search_value);
			if($search_value_replace=='')
			{
			...
			echo '<input type="text" name="search_value" placeholder="Search Anything...">';
    		...
    		echo '</div>';
			echo '</div><br/>';
			$query="select * from posts where post_heading like '%$search_value%'";
			...
			if($execute)
			{
				if(mysqli_num_rows($execute)>0)
				{
					while($row=mysqli_fetch_row($execute))
					{
						...
						if($row2[7]==null)
						{
							echo '<div class="col-xs-8" style="color:; font-size:100%;"><img height="50" class="img-circle" width="50" alt="picture" src="default_pp.png"><a href="view_profile.php?user_id='.$row[1].'">'.$row2[1].' '.$row2[2].'</a></div>';
						}
						else
						{
							echo '<div class="col-xs-8" style="color:; font-size:100%;"><img height="50" class="img-circle" width="50" alt="picture" src="data:image;base64,'.$row2[7].'"><a href="view_profile.php?user_id='.$row[1].'">'.$row2[1].' '.$row2[2].'</a></div>';
						}
						echo '<div class="col-xs-4" style="color:; font-size:100%;">'.$row[4].'<lable>views</lable>'." ".$row[5].'</div>';
						echo '</div>';
						echo '<div class="row">';
						echo '<div class="col-xs-12" style="background-color:#BBBBBB; padding-bottom:10px; padding-top:10px; border-radius:8px; font-size:150%; overflow:scroll;"><lable>'.$row[2].'</div>';
						echo '</div>';
